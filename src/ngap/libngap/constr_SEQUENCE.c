...
int SEQUENCE_constraint(
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    if (elm->encoding_constraints.general_constraints) {
      int ret = elm->encoding_constraints.general_constraints(
...
static int SEQUENCE_handle_extensions_aper(
  ...
  for (edx = specs->first_extension; edx < td->elements_count; edx++) {
    ...
    ASN_DEBUG(
        "checking %s (@%ld) present => %d", elm->type->name, edx, present);
    exts_count++;
    exts_present += present;
...
asn_enc_rval_t SEQUENCE_encode_aper(
  ...
  for (edx = 0;
    ...
    if (elm->flags & ATF_POINTER) {
      memb_ptr2 = (const void* const*) ((const char*) sptr + elm->memb_offset);
      if (!*memb_ptr2) {
        ASN_DEBUG("Element %s %ld not present", elm->name, edx);
        if (elm->optional) continue;
        /* Mandatory element is missing */
        ASN__ENCODE_FAILED;
      }
...
asn_TYPE_operation_t asn_OP_SEQUENCE = {
    ...
    0,
#else
    SEQUENCE_decode_uper, SEQUENCE_encode_uper,
