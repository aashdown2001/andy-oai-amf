...
bool amf_n2::curl_http_client_udsf(std::string remoteUrl,std::string jsonData,std::string http_mothed,nlohmann::json &udsf_response) 
{
     ...
     if (curl) {
      ...
      Logger::amf_n2().error("Cannot send HTTP message");
    }
...
void amf_n2::handle_itti_message(itti_initial_ue_message& init_ue_msg) {
  ...
  Logger::amf_n2().debug("try to get gnb_context in amf_n2.cpp");
  std::shared_ptr<gnb_context> gc;
  ...
  // try to get IEs
  Logger::amf_n2().debug("try to get IEs  in amf_n2.cpp");
  // parse ran_ue_ngap_id from INITIAL_UE_MESSAGE
  Logger::amf_n2().debug("try to parse ran_ue_ngap_id from INITIAL_UE_MESSAGE  in amf_n2.cpp");
  ...
  Logger::amf_n2().debug("try to parse nas-pdu from INITIAL_UE_MESSAGE  in amf_n2.cpp");
  uint8_t* nas_buf;
  size_t nas_len = 0;
  ...
  Logger::amf_n2().debug("try to parse location infromation from INITIAL_UE_MESSAGE  in amf_n2.cpp");
  Tai_t tai;
  NrCgi_t cgi;
  ...
  Logger::amf_n2().debug("try to parse 5g-s-tmsi from INITIAL_UE_MESSAGE  in amf_n2.cpp");
  ...
  Logger::amf_n2().debug("check if ue_ngap_context is to be establishetd a new one in amf_n2.cpp");
  bool is_ran_ue_ngap_id_new = true;
  ...
  if(itti_msg->is_5g_s_tmsi_present){
    ...
    if(amf_n1_inst->is_guti_2_nas_context_in_udsf(guti)){
      ...
      std::string udsf_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/"+ std::string("nas_context/records/") +  record_id_nas;
      if(!amf_n2_inst->curl_http_client_udsf(udsf_url,"","GET",udsf_response)){
  ...
  Logger::amf_n2().debug("try to get a ue_ngap_context using (new/old) ran_ue_ngap_id in amf_n2.cpp");
  ...
  udsf_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + record_id ;
  if(!amf_n2_inst->curl_http_client_udsf(udsf_url,"","GET",udsf_response)){
  ...
  Logger::amf_n2().debug("update ue_ngap_context to UDSF amf_n2.cpp");
  nlohmann::json udsf_ue_ngap_context;
  std::string udsf_put_url = "";
  int old_ran_ue_ngap_id = 0;
  if(is_ran_ue_ngap_id_new)
    old_ran_ue_ngap_id = ran_ue_ngap_id;
  else
    old_ran_ue_ngap_id = nc.get()->ran_ue_ngap_id;
  udsf_put_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + +"RECORD_ID=\'"+to_string(old_ran_ue_ngap_id)+"\'";
  ...
  Logger::amf_n2().debug("Send itti_nas_signalling_establishment_request to TASK_AMF_APP amf_n2.cpp");
...
# if 0
    if (!init_ue_msg.initUeMsg->get5GS_TMSI(_5g_s_tmsi)) {
      ...
      if (amf_n1_inst->check_security_header_type(type, (uint8_t *)bdata(itti_msg->nas_buf))) 
      {
        bstring decoded_plain_msg;
        switch (type) {
        case PlainNasMsg: {
          Logger::amf_n1().debug("Received plain NAS message");
          decoded_plain_msg = itti_msg->nas_buf;
        } break;
         case IntegrityProtected: {
          Logger::amf_n1().debug("Received integrity protected NAS message");
          decoded_plain_msg = blk2bstr((uint8_t *)bdata(itti_msg->nas_buf) + 7,
                                      blength(itti_msg->nas_buf) - 7);
        } break;
        default : printf("not plain nas_msg");break;
        }
        ServiceRequest *serReq = new ServiceRequest();
          serReq->decodefrombuffer(nullptr, (uint8_t *)bdata(decoded_plain_msg), blength(decoded_plain_msg));

          uint16_t amfSetId;
          uint8_t amfPointer;
          string tmsi;
          
          if(!serReq->get5G_S_TMSI(amfSetId,amfPointer,tmsi))
          {
              Logger::amf_n2().debug("5g_s_tmsi not present in nas");
          }
          else
          {
            Logger::amf_n2().debug("5g_s_tmsi set id %x, pointer %x, tmsi %s",amfSetId,amfPointer,tmsi.c_str());
            itti_msg->is_5g_s_tmsi_present = true;
            itti_msg->_5g_s_tmsi           = to_string(amfSetId) + to_string(amfPointer) +tmsi;
            Logger::amf_n2().debug("5g_s_tmsi present in nas");
            std::string guti;
            init_ue_msg.initUeMsg->getUserLocationInfoNR(cgi, tai);
            ...
            if(amf_n1_inst->is_guti_2_nas_context_in_udsf(guti))
            {
                nlohmann::json udsf_response3; 
                std::string record_id_nas = "guti=\'" + guti + "\'";
                std::string udsf_url3 = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/"+ std::string("nas_context/records/") +  record_id_nas;
                if(!amf_n2_inst->curl_http_client_udsf(udsf_url3,"","GET",udsf_response3))
                {
                  Logger::amf_n1().error("No existing nas_context with GUTI %s",guti.c_str());
                  return ;
                }
                nc.get()->nas_context_from_json(udsf_response3);
                ran_ue_ngap_id = nc.get()->ran_ue_ngap_id;
                itti_msg->amf_ue_ngap_id = nc.get()->amf_ue_ngap_id;
                if(nc == nullptr)
                {
                  printf("-----nc context ----amf n2 in nas--!!!!!!!!!!!! ");
                }
            }
  ...
  if (unc.get() == nullptr) {
    ...
    if (!init_ue_msg.initUeMsg->get5GS_TMSI(_5g_s_tmsi)) {
      ...
      //针对UERANSIM
      SecurityHeaderType type2 = {};
      if (amf_n1_inst->check_security_header_type(type2, (uint8_t *)bdata(itti_msg->nas_buf))) 
      {
        bstring decoded_plain_msg2;
        switch (type2) {
        case PlainNasMsg: {
          Logger::amf_n1().debug("Received plain NAS message");
          decoded_plain_msg2 = itti_msg->nas_buf;
          
        } break;
        case IntegrityProtected: {
          Logger::amf_n1().debug("Received integrity protected NAS message");
          decoded_plain_msg2 = blk2bstr((uint8_t *)bdata(itti_msg->nas_buf) + 7,
                                      blength(itti_msg->nas_buf) - 7);
        } break;
        default : break;
        }
        ServiceRequest *serReq2 = new ServiceRequest();
          serReq2->decodefrombuffer(nullptr, (uint8_t *)bdata(decoded_plain_msg2), blength(decoded_plain_msg2));

          uint16_t amfSetId2;
          uint8_t amfPointer2;
          string tmsi2;
          
          if(!serReq2->get5G_S_TMSI(amfSetId2,amfPointer2,tmsi2))
          {
              Logger::amf_n2().debug("5g_s_tmsi not present in nas");
          }
          else
          {
            itti_msg->is_5g_s_tmsi_present = true;
            itti_msg->_5g_s_tmsi           = to_string(amfSetId2) + to_string(amfPointer2) +tmsi2;
            Logger::amf_n2().debug("5g_s_tmsi present in nas");
            std::string guti2;

            guti2 = tai.mcc + tai.mnc + amf_cfg.guami.regionID +itti_msg->_5g_s_tmsi ;
            Logger::amf_app().debug("Receiving GUTI %s in nas", guti2.c_str());
      ...
      amf_n2_inst->curl_http_client_udsf(udsf_url_nas,udsf_nas_context.dump()," ",udsf_response);
 ...
 // 
  ...
  std::string udsf_put_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + record_id_ue_ngap ;
 ...
 // nlohmann::json udsf_response;
  ...
  // udsf_put_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + std::to_string(unc.get()->ran_ue_ngap_id) ;
...
void amf_n2::handle_itti_message(itti_ul_nas_transport& ul_nas_transport) {
  ...
  record_id = "RECORD_ID=\'" +std::to_string(ran_ue_ngap_id)  + "\'";
  std::string udsf_url_ue_ngap_context = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + record_id ;
...
void amf_n2::handle_itti_message(itti_dl_nas_transport& dl_nas_transport) {
 ...
 std::string udsf_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + record_id ;
  if(!amf_n2_inst->curl_http_client_udsf(udsf_url,"","GET",udsf_response)){
    Logger::amf_n2().error("No existing ue_ngap context with ran_ue_ngap_id ");
    return;
  }else if(udsf_response.dump().length()<8){
 ...
 //printf("---------------std::to_string(unc.get()->gnb_assoc_id) ----------------%s",std::to_string(unc.get()->gnb_assoc_id) );
  ...
  udsf_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("gnb_context/records/") + record_id ;
  if(!amf_n2_inst->curl_http_client_udsf(udsf_url,"","GET",udsf_response)){
 ...
 /***************hsx add***************/
  ...
  std::string udsf_put_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("ue_ngap_context/records/") + record_id ;
  nlohmann::json udsf_ue_ngap_context;
  udsf_ue_ngap_context["meta"] ["tags"] = {
...
std::shared_ptr<pdu_session_context> psc = std::shared_ptr<pdu_session_context>(new pdu_session_context());
 ...
 // nlohmann::json udsf_response;
  ...
  udsf_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("pdu_session_context/records/") + record_id ;
  if(!amf_n2_inst->curl_http_client_udsf(udsf_url,"","GET",udsf_response)){
...
void amf_n2::handle_itti_message(
  ...
  std::string udsf_update_url = "http://" + std::string(amf_cfg.nudsf.addr4.c_str())+":" + std::to_string(amf_cfg.nudsf.port)+ "/nudsf-dr/v1/amfdata/" + std::string("gnb_context/records/") + record_id ;
...
void amf_n2::handle_itti_message(itti_handover_required& itti_msg) {
  ...
  ngap::S_NSSAI s_nssai;
  s_nssai.setSst("01");
  std::vector<ngap::S_NSSAI> Allowed_Nssai;
